...
MODEL_PATH="$PROJECT_DIR/models/mixtral-8x7b-instruct-v0.1.Q4_K_M.gguf"
...
python3 "$PROJECT_DIR/summarize_section.py" "Key Discussion Points" "$WORKING_DIR/${BASE_NAME}_key_discussion.txt" "$WORKING_DIR/${BASE_NAME}_key_discussion_condensed.txt" "$LLAMA_RUN" "$MODEL_PATH" --ngl 99 || exit 1
STEP_END=$(date +%s)
...
python3 "$PROJECT_DIR/summarize_section.py" "Action Items and Next Steps" "$WORKING_DIR/${BASE_NAME}_action_items.txt" "$WORKING_DIR/${BASE_NAME}_action_items_condensed.txt" "$LLAMA_RUN" "$MODEL_PATH" --ngl 99 || exit 1
STEP_END=$(date +%s)
...
echo "✅ Step10 and after completed. Output saved to $OUTPUT_DIR" 